...
if ($Check -in @("Content", "All")) {
    ...
    Assert-True ($visitorMap.Contains('<span>Total Pageviews</span>')) "Visitor map must use the original English cumulative pageview label."
    Assert-True (-not $visitorMap.Contains('/ 总浏览量')) "Visitor map must not append a Chinese translation to the pageview label."
    Assert-True ($visitorMap.Contains('&t=n')) "MapMyVisitors monthly counter must be hidden when the cumulative counter is shown."
    Assert-True (Test-Path -LiteralPath (Join-Path $repoRoot "_data/visitor_stats.yml")) "Visitor statistics data file is missing."
    Assert-True (Test-Path -LiteralPath (Join-Path $repoRoot "scripts/update_visitor_stats.py")) "Visitor statistics updater is missing."
